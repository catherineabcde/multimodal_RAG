...
class Qwen3VLReranker():
    ...
    def process(self, inputs) -> list[torch.Tensor]:
        ...
        final_scores = []
        # for pair in pairs:
        #     inputs = self.tokenize([pair]) # tokenize the text/image pairs (model can read)
        ...
        for i in range(0, len(pairs), batch_size):
            ...
            t3 = time.time() 
            final_scores.extend(scores)
            print(f"  Batch {i//batch_size}: tokenize={t1-t0:.2f}s, to_device={t2-t1:.2f}s, forward={t3-t2:.2f}s")
        end_time = time.time()
        print(f"Reranking time: {end_time - start_time:.2f} seconds")
        return final_scores